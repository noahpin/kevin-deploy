...
process.exit();
      ...
      if (message.content.toLowerCase().startsWith("k.editmsg") && message.member.id == "180929397107326976") {
        ...
        if (currAction == "prod_remove") {
            ...
            if (found && message.content.startsWith("yes")) {
                ...
                if (database.products[index].type == "Keyboards") {
                    ...
                    client.channels.cache.get("720365009737220209").messages.fetch(database.kcrr[Math.floor(index / 20)]).then(msg => {
                        //const fetchedMsg = msg.first();
                        message.guild.roles.cache.get(database.products[index].roleId).delete().then(() => {
                            client.channels.cache.get(`${database.products[index].chanId}`).delete().then(() => {
                                database.products.splice(index, 1)
                    ...
                    client.channels.cache.get("720365204692795431").messages.fetch(database.mcrr[Math.floor(index / 20)]).then(msg => {
                        //const fetchedMsg = msg.first();
                        message.guild.roles.cache.get(database.products[index].roleId).delete().then(() => {
                            client.channels.cache.get(`${database.products[index].chanId}`).delete().then(() => {
                                database.products.splice(index, 1)
        ...
        if (message.content.startsWith("k.extras")) {
            ...
            message.channel.send({ files: ["https://media.discordapp.net/attachments/690304948088733789/720819482481918033/original.png?width=931&height=683"] })
